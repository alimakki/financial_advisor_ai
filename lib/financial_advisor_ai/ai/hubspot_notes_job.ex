  ...
  def perform(%Oban.Job{args: %{"user_id" => user_id}}) do
    ...
    case HubspotService.process_contact_notes(user_id) do
      {:ok, %{processed_count: count, results: results}} ->
        ...
        errors = Enum.filter(results, fn {_contact_id, result} ->
          case result do
            {:error, _} -> true
            _ -> false
          end
        ...
        if length(errors) > 0 do
          Logger.warning("Some contacts had errors during processing for user #{user_id}: #{inspect(errors)}")
        end
  ...
  def schedule_for_all_users do
    case FinancialAdvisorAi.AI.list_users_with_integrations("hubspot") do
      users when is_list(users) ->
        ...
        successful_jobs = Enum.filter(jobs, fn
          {:ok, _job} -> true
          _ -> false
        end)
